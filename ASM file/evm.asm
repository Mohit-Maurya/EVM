...
DATA_K          DW      0F001H,0A002H,0A003H,0A004H,0A005H,0A006H,0A007H,0A008H
DW      0F002H,0E009H,0E003H,0E004H,0E005H,0E006H,0E007H,0E008H
DW      0F003H,0D002H,0030H,0031H,0032H,0033H,0D003H,0D004H
DW      0F004H,0D006H,0034H,0035H,0036H,0037H,0D007H,0D008H            ;;10 11-> PAUSE RESUME
DW      0F005H,0C001H,0038H,0039H,0008H,0E002H,0C002H,0C003H
DW      0F006H,0C005H,000DH,000AH,000BH,000CH,0053H,0054H
DW      0F007H,0056H,0057H,0058H,0059H,005AH,0021H,0040H
DW      0F008H,0024H,0025H,002CH,002AH,002DH,002FH,003EH
...
;if no key press detected return from isr
...
mov di,temp1
call clear_lcd
mov        dx,0006h        ;5 characters in the password
...
Backspace       proc near
mov al,00010011b        ;setting address to increment by 1 and diplay shifting enabled i.e., entry mode set
out portA2,al            ;??? SHOULDNT IT BE 0000 0110B ?---
mov al,00100000b        ;E=0
out portC2,al
mov al, 00000000b        ;set E=1;
...
raise_alarm_2        proc near

mov al,00000010b  ;bsr mode of portc to raise the alarm led
out portC1,al
push cx
mov cx,10
z:       call sub1
...
raise_alarm_3        proc near

mov al,00000100b  ;bsr mode of portc to raise the alarm led
out portC1,al
push cx
mov cx,10
a:        call sub1
...
ret
raise_alarm_3        endp
raise_alarm_4        proc near

mov al,00001000b  ;bsr mode of portc to raise the alarm led
out portC1,al
push cx
mov cx,10
c:       call sub1
...
raise_alarm_5        proc near

mov al,00010000b  ;bsr mode of portc to raise the alarm led
out portC1,al
push cx
mov cx,10
de:        call sub1
...
raise_alarm_6        proc near

mov al,00100000b  ;bsr mode of portc to raise the alarm led
out portC1,al
push cx
mov cx,10
e:       call sub1
...
raise_alarm_7        proc near

mov al,01000000b  ;bsr mode of portc to raise the alarm led
out portC1,al
push cx
mov cx,10
f:      call sub1
...
raise_alarm_8        proc near

mov al,10000000b  ;bsr mode of portc to raise the alarm led
out portC1,al
push cx
mov cx,10
g:        call sub1
...
sub1   proc    near
push cx
mov        cx,50000 ; delay generated will be approx 0.18 secs
xat:          loop        xat
pop cx
ret
sub1       endp
